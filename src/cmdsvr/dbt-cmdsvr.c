...
static util_cmdsvr_cb_ret_t dbt_cmdsvr_cb(util_cmdsvr_conf_t *conf)
{
    ...
    while (conf->req.buff[next_arg_i] == ' ' || conf->req.buff[next_arg_i] == '\t')
        next_arg_i++;
    char *arg_str = &conf->req.buff[next_arg_i];
    // _UTIL_PRINTF("\n\ndbt_cmdsvr_cb: Calling __libdft_cmd with cmd=%d and arg_str='%s'\n", cmd_num, arg_str);
...
static void dbt_cmdsvr_init()
{
    ...
    if (pthread_attr_init(&cmdsvr_conf->pthread_attr) != 0)
    {
        _UTIL_PRINTF("Error calling pthread_attr_init()!\n");
        return;
    }
